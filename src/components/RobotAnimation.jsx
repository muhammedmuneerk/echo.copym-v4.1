...
const RobotAnimation = ({ width = '100%', height = '100%' }) => {
  ...
  useEffect(() => {
    ...
    // Libraries are imported as ES modules
    ...
    gsap.to("#graph-cir-mid-2", {
      ...
      yoyo: true
         ...
         // MorphSVG plugin animations (if available)
     // These are advanced GSAP plugins and might not be available in the free version
     try {
       gsap.to("#graph-left", {
       ...
       console.log('Advanced GSAP plugins not available, skipping morph animations');
     }
    ...
    gsap.to(".circles-top", {
      ...
      stagger: {
        ...
        repeat: -1
         ...
         // DrawSVG plugin animations (if available)
     try {
       gsap.to("#circle-l", {
         drawSVG: "20",
         repeat: -1,
       ...
       console.log('DrawSVG plugin not available, skipping draw animations');
     }
    ...
    gsap.to("#graph-line", {
     ...
     try {
       ...
       console.log('Advanced GSAP plugins not available for bar animations');
     }
    ...
    function initStars() {
      ...
      scene.add(stars);
       ...
       renderer.setClearColor("#282e39", 0.3);
       renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
      
      const webglElement = container.querySelector("#webgl");
      if (webglElement) {
    ...
    const canvas = container.querySelector("canvas.webgl2");
    if (canvas) {
    ...
    const robotElement = container.querySelector("#robot");
    if (robotElement) {
    ...
    const handleMouseMove = (e) => {
     ...
     // Resize handler for responsive background
     const handleResize = () => {
       if (camera && renderer) {
     ...
     return () => {
       ...
       if (robotElement) {
         robotElement.removeEventListener("click", () => {});
       }
     };
  }, []);
     ...
     <div 
       ref={containerRef} 
       className="robot-animation-container"
     >
      <div id="webgl"></div>
      <div id="bg-box">
