...
/* ------------------------------------------------------------------
   Shared futuristic animations & effects
-------------------------------------------------------------------*/

// Subtle animated gradient sweep that travels diagonally across the card
...
// Pulsating glow for borders / shadows
...
// Gentle shimmer for glass-like reflection
const shimmerAnimation = keyframes`
  0%   { background-position: -400px 0; }
  100% { background-position: 400px 0; }
`;

// Floating reflection travelling across the surface
...
/* ------------------------------------------------------------------
   Legacy border animations (kept for backward compatibility)
-------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------
   Futuristic Animated Card wrapper – exported component
-------------------------------------------------------------------*/
const AnimatedCardWrapper = styled(Box)(({ theme }) => ({
  ...
  /* Child utility classes preserved for compatibility */
  '& .glass-reflection': {
    position: 'absolute',
    top: 0,
    left: 0,
...
/* ------------------------------------------------------------------
   Inner Card wrapper – colour-adaptive variant (used by sliders)
-------------------------------------------------------------------*/
const InnerCardWrapper = styled(Box)(({ theme, color }) => {
  const neon = color || 'rgba(0,224,255,1)';
  return {
    position: 'relative',
    width: '100%',
    height: '100%',
    ...
    '&:hover': {
      transform: 'rotateX(6deg) rotateY(-6deg) translateY(-6px)',
      borderColor: `${neon}55`,
      boxShadow: `0 0 22px ${neon}44, 0 18px 38px rgba(0,0,0,.75)`,
      animation: `${neonPulse} 2.8s ease-in-out infinite`,
      '& .glass-reflection': { animation: `${glassReflection} 3s linear infinite` },
      '& .card-content': { transform: 'translateZ(12px)' },
      '& .border-right': { animation: `${borderAnimationRight} 2.5s linear infinite` },
      '& .border-down':  { animation: `${borderAnimationDown} 2.5s linear infinite`, animationDelay: '1s' },
      '& .border-left':  { animation: `${borderAnimationLeft} 2.5s linear infinite`, animationDelay: '1.5s' },
      '& .border-up':    { animation: `${borderAnimationUp} 2.5s linear infinite`, animationDelay: '2s' },
...
export {
  AnimatedCardWrapper,
  InnerCardWrapper,
  // legacy exports (unchanged names)
  borderAnimationRight,
  borderAnimationDown,
  borderAnimationLeft,
  borderAnimationUp,
  shimmerAnimation,
